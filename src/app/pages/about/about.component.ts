...
import {
  mauroDataMapperVersionStatusKey,
  nhsDataDictionaryPluginModuleKey
} from '@mdm/mdm-resources/mdm-resources/adapters/mdm-admin.model';
...
export class AboutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.isSignedIn) {
      ...
      forkJoin([this.admin.status(), this.admin.modules()])
        .pipe(finalize(() => (this.isLoading = false)))
        .subscribe(([status, modules]) => {
          this.mauroVersion = status[mauroDataMapperVersionStatusKey];
          this.nhsDdPluginVersion =
            modules.find((module) => module.name === nhsDataDictionaryPluginModuleKey)
              ?.version ?? '';
        });
    }
  }
